...
public class InventoryService {
    ...
    public InvoiceViewModel saveInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoice.getItemType().equals("Consoles")) {
            ...
            Game gamequan = gameDao.getGame(game.getQuantity());
            unitprice = game.getPrice();
            quantity = game.getQuantity();

            quantityAtHand = gamequan.getQuantity();
            ...
            TShirt tquan = tShirtDao.getTShirt(tShirt.getQuantity());
            unitprice = tShirt.getPrice();
            quantity = tShirt.getQuantity();

            quantityAtHand = tquan.getQuantity();
...
//calculting the subTotal and Total based on the quantity and unit_price.
        ...
        if (invoice.getItemType().equals("console")) {
            Console console = consoleDao.getConsole(invoice.getItemId());
            Console conquan = consoleDao.getConsole(console.getQuantity());
            unitprice = console.getPrice();
            quantity = console.getQuantity();
            quantityAtHand = conquan.getQuantity();
